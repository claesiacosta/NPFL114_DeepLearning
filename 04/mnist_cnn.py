...
class Network(tf.keras.Model):
    def __init__(self, args):
        ...
        def cnn_layers(hidden, params):
            if params[0] == 'C':
                return tf.keras.layers.Conv2D(filters=int(params[1]), kernel_size=int(params[2]), strides=int(params[3]), padding=params[4], activation='relu')(hidden)
            elif params[0] == 'CB':
                hidden = tf.keras.layers.Conv2D(filters=int(params[1]), kernel_size=int(params[2]), strides=int(params[3]), padding=params[4], use_bias=False)(hidden)
                hidden = tf.keras.layers.BatchNormalization()(hidden)
                return tf.keras.layers.Activation('relu')(hidden)
            elif params[0] == 'M':
                return tf.keras.layers.MaxPool2D(pool_size=int(params[1]), strides=int(params[2]), padding='valid')(hidden)
            elif params[0] == 'F':
                return tf.keras.layers.Flatten()(hidden)
            elif params[0] == 'H':                
                return tf.keras.layers.Dense(int(params[1]), activation='relu')(hidden)
            elif params[0] == 'D':                
                return tf.keras.layers.Dropout(float(params[1]))(hidden)

        hidden = inputs
        
        for params in def_params:
            if params[0] == 'R':
                prev = hidden
                temp = params[3:-1].split(",")
                for tem in temp:
                    hidden = cnn_layers(hidden, tem.split('-'))
                hidden = tf.add(hidden, prev)
            else:
                params = params.split('-')
                hidden = cnn_layers(hidden, params) 
