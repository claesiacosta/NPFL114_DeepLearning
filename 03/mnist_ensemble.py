...
def main(args):
    ...
    for model in range(args.models):
        ...
        models.append(
            tf.keras.Sequential([tf.keras.layers.Flatten(input_shape=[MNIST.H, MNIST.W, MNIST.C]),] +
    ...
    for model in range(args.models):
        ...
        # individual_accuracy = models[model].evaluate(mnist.train.data["images"], mnist.train.data["labels"], batch_size=args.batch_size)[1]
        ...
        for m in models[0:model+1]:
            prediction = m.predict(mnist.dev.data["images"], batch_size=args.batch_size)
            predictions = prediction + predictions if predictions is not None else prediction

        predictions = predictions / (model + 1)

        acc = tf.metrics.SparseCategoricalAccuracy()
        acc.update_state(mnist.dev.data["labels"], predictions)
