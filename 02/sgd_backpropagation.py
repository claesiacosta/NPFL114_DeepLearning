'''
Antonia Claésia da Costa Souza - 80acc802-7cec-11eb-a1a9-005056ad4f31
...
class Model(tf.Module):
    ...
    def predict(self, inputs):
        ...
        res = tf.reshape(inputs, [inputs.shape[0], -1])
    ...
    def train_epoch(self, dataset):
        for batch in dataset.batches(self._args.batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                loss = tf.reduce_mean(
                    tf.reduce_sum(
                        -tf.one_hot(batch["labels"],
                        MNIST.LABELS) * tf.math.log(probabilities),
                        axis=1
                    )
                )
