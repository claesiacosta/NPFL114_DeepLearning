    ...
    def __call__(self):
        ...
        while True:
            ...
            for index, label in enumerate(
                    self._generator.choice(self._unique_labels, size=self._args.classes, replace=False)):
                indices.extend(
                    self._generator.choice(self._label_indices[label], size=self._args.images_per_class, replace=False))
...
class Network(tf.keras.Model):
    ...
    class MemoryAugmentedLSTM(tf.keras.layers.AbstractRNNCell):
        ...
        def state_size(self):
            ...
            return self._controller.state_size, self._read_heads * self._cell_size, (
            self._memory_cells, self._cell_size)
        ...
        def call(self, inputs, states):
            ...
            controller_output, controller_state = self._controller(tf.concat((inputs, read_value), axis=-1),
                                                                   controller_state)
            ...
            parameters_output = self._parameters(controller_output)
            write_value = parameters_output[:, :self._cell_size]
            read_keys = parameters_output[:, self._cell_size:]
            read_keys = tf.reshape(read_keys, [-1, self._read_heads, self._cell_size])
            ...
            normalized_read_keys = tf.math.l2_normalize(read_keys, axis=-1)
            attention = tf.linalg.matmul(normalized_read_keys, normalized_memory, transpose_b=True)
            softmax = tf.nn.softmax(attention, axis=-1)
            read_value = tf.reshape(tf.linalg.matmul(softmax, memory), [-1, self._read_heads * self._cell_size])
        ...
        def get_initial_state(self, inputs=None, batch_size=None, dtype=None):
            _, read_value, memory = self.state_size
            return (
                    self._controller.get_initial_state(inputs, batch_size, dtype),
                    tf.zeros([batch_size, read_value], dtype=tf.float32),
                    tf.zeros([batch_size, *memory], dtype=tf.float32)
                )
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        x = tf.keras.layers.Conv2D(filters=8, kernel_size=(3, 3), strides=2, padding='valid')(images)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.BatchNormalization())(x)
        x = tf.keras.layers.ReLU()(x)

        x = tf.keras.layers.Conv2D(filters=16, kernel_size=(3, 3), strides=2, padding='valid')(x)
        x = tf.keras.layers.TimeDistributed(tf.keras.layers.BatchNormalization())(x)
        x = tf.keras.layers.ReLU()(x)
        ...
        x = tf.keras.layers.Reshape(target_shape=(-1, 128))(x)
        ...
        encoded = tf.one_hot(previous_labels, depth=args.classes)
        input_to_memory_augmented_lstm = tf.concat((x, encoded), axis=-1)
        ...
        cell = self.MemoryAugmentedLSTM(args.lstm_dim, args.classes * args.images_per_class, args.cell_size,
                                        args.read_heads)
        rnn = tf.keras.layers.RNN(cell, return_sequences=True)
        x = rnn(input_to_memory_augmented_lstm)
        ...
        predictions = tf.keras.layers.Dense(args.classes, activation=tf.nn.softmax)(x)
