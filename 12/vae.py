...
class VAE(tf.keras.Model):
    def __init__(self, args):
        ...
        for l_size in args.encoder_layers:
        ...
        self.encoder = tf.keras.Model(inputs=img_input, outputs=[z_mean, z_log_variance])
        ...
        self.decoder = tf.keras.Model(inputs=vec_input, outputs=output)
    ...
    def train_step(self, images):
        with tf.GradientTape() as tape:
            ...
            dist = tfp.distributions.Normal(z_mean, tf.math.exp(z_log_variance / 2))
            ...
            latent_loss = tf.reduce_mean(tfp.distributions.kl_divergence(dist, tfp.distributions.Normal(loc=0, scale=1)))
