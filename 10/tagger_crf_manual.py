...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        # Add type hints to make pycharm shut up
        args.we_dim: int
        args.logdir: str
        args.rnn_cell: str
        args.rnn_cell_dim: int
        ...
        embedded = tf.keras.layers.Embedding(vocab_size, args.we_dim)(indices)
        ...
        else:  # if args.rnn_cell == "GRU"
            hidden = tf.keras.layers.GRU(args.rnn_cell_dim, return_sequences=True)

        hidden = tf.keras.layers.Bidirectional(hidden, merge_mode="sum")(embedded.to_tensor(),
                                                                         mask=tf.sequence_mask(embedded.row_lengths()))
        hidden = tf.RaggedTensor.from_tensor(hidden, embedded.row_lengths())
        ...
        predictions = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(num_tags, activation=None))(hidden)
        ...
        self._crf_weights = self.add_weight(shape=[num_tags, num_tags], initializer=tf.initializers.Zeros(),
    ...
    def crf_loss(self, gold_labels, logits):
        ...
        outputs = logits.to_tensor()
        tags = gold_labels.to_tensor()
        seq_lengths = gold_labels.row_lengths()

        tags = tf.cast(tags, dtype=tf.int32)
        seq_lengths = tf.cast(seq_lengths, dtype=tf.int32)
        crf_weights = tf.cast(self._crf_weights, dtype=outputs.dtype)

        # Extract input sizes
        batch_size, max_seq_len, tags_len = tf.shape(outputs)[0], tf.shape(outputs)[1], tf.shape(outputs)[2]
        num_transitions = tf.shape(tags)[1] - 1

        def calculate_unary_scores():

            offsets = tf.range(batch_size * max_seq_len) * tags_len

            outputs_flat = tf.reshape(outputs, [-1])

            tag_indices_flat_offset = offsets + tf.reshape(tags, [-1])
            relevant = tf.gather(outputs_flat, tag_indices_flat_offset)
            relevant = tf.reshape(relevant, [batch_size, max_seq_len])

            masks = tf.sequence_mask(seq_lengths, maxlen=tf.shape(tags)[1], dtype=relevant.dtype)
            unary_scores = tf.reduce_sum(relevant * masks, axis=1)

            return unary_scores

        unary_scores = calculate_unary_scores()

        def calc_binary_scores():

            tag_i_start = tf.slice(tags, begin=[0, 0], size=[-1, num_transitions])
            tag_i_end = tf.slice(tags, begin=[0, 1], size=[-1, num_transitions])

            weight_indices_flat = tag_i_start * tags_len + tag_i_end
            weight_params_flat = tf.reshape(crf_weights, [-1])

            binary_scores = tf.gather(weight_params_flat, weight_indices_flat)

            masks = tf.sequence_mask(seq_lengths, maxlen=tf.shape(tags)[1], dtype=binary_scores.dtype)
            truncated_masks = tf.slice(masks, [0, 1], [-1, -1])
            binary_scores = tf.reduce_sum(binary_scores * truncated_masks, 1)

            return binary_scores

        binary_scores = calc_binary_scores()

        sequence_scores = unary_scores + binary_scores

        def crf_layer():
            output_begin = tf.slice(outputs, [0, 0, 0], [-1, 1, -1])
            output_begin = tf.squeeze(output_begin, [1])
            output_rest = tf.slice(outputs, [0, 1, 0], [-1, -1, -1])

            last_index = tf.maximum(tf.constant(0, dtype=seq_lengths.dtype), seq_lengths - 1)
            output_rest = tf.transpose(output_rest, [1, 0, 2])
            weights = tf.expand_dims(crf_weights, 0)

            def func(prev, x):
                prev = tf.expand_dims(prev, 2)
                transition_scores = prev + weights
                return x + tf.reduce_logsumexp(transition_scores, [1])

            alphas = tf.transpose(tf.scan(func, output_rest, output_begin), [1, 0, 2])
            alphas = tf.concat([tf.expand_dims(output_begin, 1), alphas], 1)
            idxs = tf.stack([tf.range(tf.shape(last_index)[0]), last_index], axis=1)
            alphas = tf.gather_nd(alphas, idxs)
            return alphas

        alphas = crf_layer()

        norm = tf.reduce_logsumexp(alphas, [1])
        norm = tf.where(tf.less_equal(seq_lengths, 0), tf.zeros_like(norm), norm)

        self._crf_weights = crf_weights

        return tf.reduce_mean(norm - sequence_scores)
    ...
    def crf_decode(self, logits):
        ...
        logitsT = logits.to_tensor()
        predictions, score = tfa.text.crf_decode(logitsT, self._crf_weights, logits.row_lengths())
        predictions = tf.RaggedTensor.from_tensor(predictions, lengths=logits.row_lengths())
...
def main(args):
    ...
    # example = [i for i in train.take(1)][0]
    # outputs = network(example[0])
    # network.crf_loss(example[1], outputs)
