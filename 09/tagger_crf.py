...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        num_chars = train.forms.char_mapping.vocab_size()
        ...
        hidden = tf.keras.layers.Bidirectional(hidden, merge_mode="sum")(embed_words.to_tensor(),mask=tf.sequence_mask(embed_words.row_lengths()))
        hidden = tf.RaggedTensor.from_tensor(hidden, embed_words.row_lengths())
        ...
        self._crf_weights = self.add_weight(shape=[num_tags, num_tags], initializer=tf.initializers.Zeros(), dtype=tf.float32, trainable=True)
    ...
    def crf_loss(self, gold_labels, logits):
        ...
        gold_labelsT = gold_labels.to_tensor()
        log_likelihood, self._crf_weights = tfa.text.crf_log_likelihood(inputs=logitsT, tag_indices=gold_labelsT, sequence_lengths=gold_labels.row_lengths(), transition_params=self._crf_weights)
