...
class Network(tf.keras.Model):
    def __init__(self, args):
        ...
        regu = tf.keras.regularizers.L1L2(0,0.01)
        hidden = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(150, return_sequences = True, kernel_regularizer=regu, recurrent_regularizer=regu, bias_regularizer=regu,), merge_mode="sum")(inputs.to_tensor(), mask=tf.sequence_mask(inputs.row_lengths()))
        #hidden2 = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(150, return_sequences = True, kernel_regularizer=regu, recurrent_regularizer=regu, bias_regularizer=regu,), merge_mode="sum")(inputs.to_tensor(), mask=tf.sequence_mask(inputs.row_lengths()))
        #hidden = tf.keras.layers.Add()([hidden, hidden2])
        
        hidden = tf.RaggedTensor.from_tensor(hidden, inputs.row_lengths())
    

        logits = tf.keras.layers.TimeDistributed(tf.keras.layers.Dense(1 + len(CommonVoiceCs.LETTERS), activation=None))(hidden)
    ...
    def ctc_loss(self, gold_labels, logits):
        ...
        #gold_l_ind = tf.where(tf.not_equal(gold_labelsT, 0))
        #gold_labelsS = tf.sparse.SparseTensor(gold_l_ind, tf.cast(tf.gather_nd(gold_labelsT, gold_l_ind), tf.int32), tf.shape(gold_labelsT, tf.int64))
        gold_labelsS = tf.cast(gold_labels.to_sparse(), tf.int32)
        loss = tf.nn.ctc_loss(gold_labelsS, logitsT, None, tf.cast(logits.row_lengths(), tf.int32), logits_time_major=False, blank_index=len(CommonVoiceCs.LETTERS))
    ...
    def ctc_decode(self, logits):
        ...
        logitsT = tf.transpose(logits.to_tensor(), perm=[1, 0, 2])
        (predictions,), _= tf.nn.ctc_beam_search_decoder(logitsT, sequence_length=tf.cast(logits.row_lengths(), tf.int32), beam_width=self._ctc_beam)
...
def main(args):
    ...
    def create_dataset(name):
        def prepare_example(example):
            ...
            split_exa = tf.strings.unicode_split(sentences, input_encoding='UTF-8')
     
            return example["mfccs"], cvcs.letters_mapping(split_exa)
